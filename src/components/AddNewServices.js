  ...
  return (
    <div class={`${props.showAddServiceBox === true ? "flex" : "hidden" } fixed w-full h-full bg-slate-500/60 items-center justify-center z-10`}>
      <div class="fixed flex flex-col gap-6 items-center justify-center bg-white p-24 rounded-xl z-10 drop-shadow-md">
          ...
          <input type="text" {...register("name", {required: true})} id='serviceName' class='border rounded-md p-2 mr-2' />
          <label htmlFor="servicePrice">Price:</label>
          {errors.price && <span>This field is required</span>}
          <input type="number" {...register("price", {required: true})} id='servicePrice' class='border rounded-md p-2 mr-2'/>
          <label htmlFor="serviceDuration">Duration:</label>
          {errors.duration && <span>This field is required</span>}
          <input type="number" {...register("duration", {required: true})} id='serviceDuration' class='border rounded-md p-2 mr-2'/>
          <input value={"Add service"} type='submit' class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" />
          </form>
      </div>
    </div>
  )
